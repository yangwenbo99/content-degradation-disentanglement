...
class ImageCompressor(nn.Module):
    def __init__(self, out_channel_N=128, in_channel_N=3):
        super(ImageCompressor, self).__init__()
        self.Encoder = Analysis_net_17(out_channel_N=out_channel_N, in_channel_N=in_channel_N)
        self.Decoder = Synthesis_net_17(out_channel_N=out_channel_N, in_channel_N=in_channel_N)
        self.bitEstimator = BitEstimator(channel=out_channel_N)
        self.out_channel_N = out_channel_N

    def forward(self, input_image, bitrate_map=False):
        ...
        # distortion
        mse_loss = torch.mean((recon_image - input_image).pow(2), dim=(-1, -2, -3))
