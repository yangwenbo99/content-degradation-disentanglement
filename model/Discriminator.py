...
from .SubBlocks import conv_down, conv3x3

class DiscriminatorLinear(nn.Module):
    def __init__(self, in_chn, ndf=64, slope=0.2, in_width=128):
        '''
        ...
        if in_width >= 128: 
            main_module.append(conv_down(in_chn, ndf, bias=False))
        else: 
            main_module.append(conv3x3(in_chn, ndf, bias=False))
        ...
        if in_width >= 64: 
            ...
            main_module.append(conv3x3(ndf, ndf*2, bias=False))
        main_module.append(nn.LeakyReLU(slope, inplace=True))
        # state size: N x (ndf*2) x 32 x 32
        if in_width >= 32: 
            main_module.append(conv_down(ndf*2, ndf*4, bias=False))
        else: 
            main_module.append(conv3x3(ndf*2, ndf*4, bias=False))
        main_module.append(nn.LeakyReLU(slope, inplace=True))
