...
class LPIPSvgg(torch.nn.Module):
    def __init__(self, channels=3):
        ...
        self.weights = torch.load(os.path.abspath(os.path.join(inspect.getfile(DISTS), '..', 'weights/LPIPSvgg.pt')))
    ...
    def forward(self, x, y, as_loss=True):
        ...
        for k in range(len(self.chns)):
            score = score + (self.weights[k] * (feats0[k] - feats1[k])**2).mean([2, 3]).sum(1)
        return score
